...
- **接口地址**: `GET /api/schedule/job/log/job/{jobId}`
- **功能描述**: 根据任务ID获取对应的执行记录列表
- **路径参数**:
...
- **接口地址**: `GET /api/schedule/job/log/page`
- **功能描述**: 获取调度任务执行记录分页列表，支持按条件查询
- **请求参数**:
...
| current | Long | 否 | 当前页码，默认为1 |
| size | Long | 否 | 每页显示条数，默认为10 |
| workflowId | String | 否 | 工作流ID（作为jobId查询条件） |
